...
MetaCentrum (within e-INFRA CZ) also operates a container platform based on Kubernetes, including a Rancher-based distribution and related user-facing tooling \cite{EinfraHPCKubernetes}. This is relevant for PBSmon for two reasons. First, Kubernetes-based services are part of the broader infrastructure portfolio and strengthen the requirement for modularity and extensibility of monitoring solutions. Second, if monitoring is unified across multiple compute paradigms (PBS grid and Kubernetes clusters), future PBSmon modules may benefit from shared concepts (service health, resource utilization, user-to-project mapping) even if the underlying schedulers differ \cite{EinfraHPCKubernetes,KubernetesDocs}.
