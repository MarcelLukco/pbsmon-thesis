...
The primary goal of the application is to provide a view about current state of the MetaCentrum infrastructure.

Instead, data must be collected periodically by dedicated collectors and stored in a cache. The resulting data should be proccessed and served to the end users from this cache, which keeps user-facing requests fast and predictable while decoupling them from slow or heavy collection tasks.
...
As mentioned in the section~\ref{sec:authentication}, the old PBSMon solution uses third-party authentication provided by Perun's e-INFRA system. The new solution uses same third-party authentication using the same OIDC authentication protocol.

However, the new solution introduces two role system - administator and user. It was decided
that those users, which belong either to group metasw or pbs-admins will be considered administrators and have full access to all data.
