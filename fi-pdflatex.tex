...
In addition to PBS- and Perun-derived datasets, the system collects selected operational and inventory information related to the OpenStack environment. These data are obtained from a Prometheus server, which provides metrics and metadata exported by infrastructure components and custom exporters \cite{PrometheusOverview}. 

Unlike PBS and Perun data (which are loaded from files and uses filesystem as form of communication), the Prometheus-derived data are obtained from prometheus, and since the prometheus provides API endpoint for querying data, it does not need any complex setup. Therefore it was decided that prometheus metrics will not be persisted to the filesystem. Since the selected technology NestJS supports native setup of CRON jobs, the cron jobs, that occurs on API start and then every ten minutes will obtain data from prometheus and store in memory.  Therefore ensuring that the latest successfull snapshot will be stored in memory.
