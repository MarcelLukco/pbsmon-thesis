...
\thesissetup{ 
    ...
    keywords    = {cloud, cloud computing, distributed computing, MetaCentrum, NestJS, React},
    TeXkeywords = {cloud, cloud computing, distributed computing, MetaCentrum, NestJS, React},
    abstract    = {%  
    This thesis describes the development of a new web application intended to replace the existing PBSMon monitoring tool used in the MetaCenter environment. The work begins with an analysis of the original solution, its limitations, and relevant application and technologies for MetaCenter infrastructure. Based on the identified requirements, the thesis proposes a new architecture and justifies key design decisions, including the separation into multiple cooperating components. All of this is followed by the describing of the implementation results of the new solution. The thesis concludes by outlining possible future improvements and extensions.
    },
    declaration = {%
...
\begin{itemize}
  ...
  \item Cursor - code assistant for faster code writing,
...
PBSMon collects data from the following environments:
\begin{itemize}
...
Each of these environments has its own method of communication and data representation. Within the system architecture, these differences are abstracted through interfaces and adapters that ensure unified data processing. Environments and their data structure are described in the following sections.
...
The presentation of data is performed through server-side rendering of HTML pages. Only logged-in users, who have approved access to PBSMon can access the web interface. The authorization is performed by third party system using OIDC token, that will be described in detail in section \ref{sec:authentication}.
...
\begin{itemize}
  ...
  \item \textbf{Resources and default settings} -- manages available/assigned resources and defaults and provides summary utilization (assigned memory/CPU/nodes).
...
\begin{itemize}
  \item \textbf{Identity and type} -- queue name, queue\_type, priority (weight in scheduling), operational state (enabled/started), and optionally has nodes.

  \item \textbf{State and utilization} -- 
  aggregates such as total jobs and state count, and currently
  assigned resources (e.g., memory, CPUs, nodes, MPI processes).
  ...
  \item \textbf{Access and security} -- allows to toggle access to the queue for multiple ACL levels - user, group, host.
...
\begin{itemize}
  ...
  \item \textbf{Assigned resources} -- currently allocated resources tracked by \texttt{resources\_assigned.*} attributes, including same attributes as in available resources.
  ...
  \item \textbf{Timestamps} -- last state change time and last used time. \cite{pbs2022}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Type} -- the data type of the resource value, which determines how the resource is interpreted and validated. Eq. 
  \texttt{long} -- integer values (e.g., CPU count, GPU count),
  \texttt{size} -- memory or storage values,
  \texttt{string} -- text values,
  \texttt{float} -- floating-point numeric values,
  \texttt{time} -- time duration values.
  
  \item \textbf{Flag} -- a string of characters indicating where the resource can be used or referenced - \texttt{h} at the host/node level, \texttt{q} at the queue level, \texttt{n} at the node level, \texttt{m} at the server level.\cite{pbs2022} 
\end{itemize}
...
In addition to management of users, Perun collects information about physical computing resources of the MetaCenter infrastructure.
These data are then passed to PBSMon from Perun. Data are hierarchically structured and grouped by Organization -> Cluster -> Computing Node.
...
These data are necessary to get additional information about the complete information about the computing node for the running PBS jobs.
Additionally, these data independently from PBS are important for knowing the complete information about the computing nodes for whole MetaCenter infrastructure.


\subsection{Data Collection Mechanism}
\label{subsec:data-collection-mechanism-perun}

The integration of the PBSMon system with Perun is designed using a PUSH model. Instead of direct access to the Perun database or invoking its API, Perun periodically generates JSON files containing all relevant information about users and computing resources. These files are then transferred via SSH directly to the PBSMon server, where they are stored in the filesystem, and subsequently loaded and processed by PBSMon. These files are stored in the filesystem as follows:
...
The primary goal of the application is to provide a view of current state of the MetaCentrum infrastructure.

Instead, data must be collected periodically by dedicated collectors and stored in a cache. The resulting data should be processed and served to the end users from this cache, which keeps user-facing requests fast and predictable while decoupling them from slow or heavy collection tasks.

\begin{itemize}
  \item \textbf{Freshness.} The cached dataset should be refreshed regularly so that the UI reflects the current state with acceptable delay.
  \item \textbf{Performance.} User requests must be handled quickly by reading from the cache, without triggering expensive collection work.
  \item \textbf{Load protection.} Collection and external-system access must be rate-limited and scheduled to prevent overload and denial-of-service scenarios.
  \item \textbf{Reliability.} Temporary failures of external systems should not break the UI; the last successfully collected dataset should remain available.
  \item \textbf{Scalability.} The architecture should handle increasing data volume and user traffic.
\end{itemize}
...
As mentioned in the section~\ref{sec:authentication}, the old PBSMon solution uses third-party authentication provided by Perun's e-INFRA system. The new solution uses the same third-party authentication using the OIDC authentication protocol.

However, the new solution introduces two role system - administator and user. It was decided
that those users, who belong either to group metasw or pbs-admins will be considered administrators and have full access to all data.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \texttt{config/} -- application and environment configuration. e.g. QSUB assembler configuration, pbs server list, etc.
    \item \texttt{modules/} -- feature-oriented modules. Each module have a domain entity and its related data. Each module consists of controllers and services. And therefore each module have its own routes and endpoints. There are list of modules:
    \begin{itemize}
...
Perun is not collected by PBSMon in the same way as PBS or OpenStack, because the required data are provided to PBSMon via a push mechanism external to the application. For this reason, PBSMon does not implement an active Perun client; instead, it only consumes prepared Perun exports. Since the push mechanism does not require specific requirement, it was not neccessary to create a dedicated microservice for Perun collection
...
In addition to PBS- and Perun-derived datasets, the system collects selected operational and inventory information related to the OpenStack environment. Unlike PBS and Perun data (which are loaded from files and uses filesystem as form of communication), the OpenStack data are obtained from prometheus, and since the prometheus provides API endpoint for querying data, it does not need any complex setup. Therefore it was decided that prometheus metrics will not have a dedicated micorservices and will not be persisted to the filesystem. Since the selected technology NestJS supports setup of CRON jobs, the cron jobs, that occurs on API start and then every ten minutes will obtain data from prometheus and store in memory.  Therefore ensuring that the latest successfull snapshot will be stored in memory.

The collection logic is implemented in the \texttt{Prometheus\-Collection\-Service}. The service defines a fixed set of prometheus queries that cover both hardware-related metrics (CPU, memory, GPU, disks, networking) and OpenStack-specific inventory data (projects and VM's). The collecting proccess iterates over the configured queries and executes them sequentially. Each query result is stored under a human-readable key (the query \texttt{name}) together with a timestamp representing the time of the snapshot creation. If a query fails, the error is logged and collection continues, so that partial data can still be exposed to the frontend.

\subsection{Documentation}
\label{sec:api-documentation}
The API contract is described using an OpenAPI specification, which defines endpoints along with their parameters and response schemas. In addition to the static specification, the implementation exposes a Swagger UI that renders the OpenAPI document. It provides an interactive interface for exploring and testing the API. This allows developers and administrators to call individual endpoints directly from the browser and inspect responses. They can also verify the expected behaviour without requiring external tools. The OpenAPI specification is included in the thesis archive.
...
\begin{figure}[t]
  ...
  \caption{Users page (administrator view) with pagination, search, ordering, and impersonation action. (screenshot was anonymized manually to protect sensitive data)}
  \label{fig:ui-users}
\end{figure}
...
\begin{itemize}
  ...
  \item \texttt{GET /accounting/users/:username} -- aggregated historical jobs statistics.
...
The objectives of this thesis were fulfilled by designing and implementing a new web application intended to replace the existing PBSMon monitoring tool. The resulting solution provides a modern, maintainable system for presenting the state of the computing infrastructure. 

However in contrast to the old solution, the new solution shows GPU usage of PBS jobs and introduces two user roles - administrator, which can see all unanonymised data and user, which can see only anonymised data. In addition to that, it shows a more detailed view of OpenStack entities.
...
Finally, the app was deployed and is available for opponent and testing. The source code is available within open source git repository, \url{https://github.com/MarcelLukco/pbsmon-2.0}.

Future work should primarily target extending the system beyond PBS and OpenStack by adding a dedicated Kubernetes module for the MetaCenter Kubernetes service, including both a new collector and corresponding UI views. Additional improvements include providing administrative CSV exports for selected views (e.g., jobs and nodes), displaying user storage quotas to complement compute usage in personal and user detail pages, and extending the backend API with support for PBS job submission.
